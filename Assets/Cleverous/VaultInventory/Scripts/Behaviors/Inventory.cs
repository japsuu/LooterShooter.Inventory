...
namespace Cleverous.VaultInventory.Scripts.Behaviors
{
    ...
    public partial class Inventory : NetworkBehaviour
    {
        ...
        /// <summary>
        /// It's fairly rare that you genuinely need access to this. You should avoid editing the Content directly since it will not trigger all of the necessary calls to maintain the inventory state.
        /// </summary>
        /// <returns>The raw content list of the inventory.</returns>
        public List<RootItemStack> Content { get; set; }
        ...
        /// <summary>
        /// Get all content indexes using the specified SlotRestriction.
        /// </summary>
        /// <param name="sr">The SlotRestriction you want matched.</param>
        /// <param name="includeNoRestriction">Optionally include slots that have no restriction.</param>
        /// <returns>All indexes in this Inventory's configuration which use the given SlotRestriction</returns>
        public virtual int[] GetAllIndexesWithRestriction(SlotRestriction sr, bool includeNoRestriction)
        {
            List<int> result = new List<int>();
            for (int i = 0; i < Configuration.SlotRestrictions.Length; i++)
            {
                if (Configuration.SlotRestrictions[i] == null)
                {
                    if (includeNoRestriction || sr == null) result.Add(i);
                }
                else if (sr != null && Configuration.SlotRestrictions[i].GetDbKey() == sr.GetDbKey())
                {
                    result.Add(i);
                }
            }
        ...
        public virtual RuntimeItemProxy DoDiscard(int index)
        {
            ...
            // spawn new
            RuntimeItemProxy result = General.VaultInventory.SpawnWorldItem(Content[index].Source, InventoryOwner.MyTransform.position, Content[index].StackSize);
        ...
        /// <summary>
        /// Immediately split a stack in half. One empty slot is required.
        /// </summary>
        ...
        public virtual bool DoSplit(int index)
        {
            ...
            // Try adding it.
            int oudex = DoAdd(new RootItemStack(Content[index].Source, amountRemovedFromSlot), false);
        ...
        // Public Reads
        /// <summary>
        /// Local Read only. Check if any slot with the given restriction is empty.
        /// </summary>
        /// <param name="restriction"></param>
        /// <returns>True if a slot of the restriction type is empty and available.</returns>
        public virtual bool HasAnyOpenSlot(SlotRestriction restriction)
        {
            return GetValidNullIndex(restriction) != -1;
        }

        /// <summary>
        /// Local Read only. Check if the given stack can completely fit into the inventory. (This is basically a passive DoAdd() call).
        /// </summary>
        /// <param name="stack">The content to merge in</param>
        /// <returns>True if the stack fits. False otherwise.</returns>
        public virtual bool StackCanFit(RootItemStack stack)
        {
            if (!IsInitialized) return false;
            ...
            if (stack.Source.MaxStackSize > 1)
            {
                ...
                while (index < Content.Count)
                {
                    ...
                    int mergeIndex = GetIndexOfItemNotFull(stack.Source.GetDbKey(), index);
            ...
            return GetValidNullIndex(stack.Source.Restriction) != -1;
        }
