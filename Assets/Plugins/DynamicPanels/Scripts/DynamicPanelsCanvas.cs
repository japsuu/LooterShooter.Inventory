...
#pragma warning disable 0649
		[Tooltip("when enabled, there will always be some free space in the canvas that docked panels can't fill. Otherwise, docked panels will fill the whole canvas")]
		[SerializeField]
		...
		[Tooltip("if Leave Free Space is enabled and a RectTransform is assigned to this variable, the RectTransform's properties will automatically be updated to always match the free space's position and size")]
		[SerializeField]
		...
		[Tooltip("size of the invisible resize zones at each side of the panels that allow users to resize the panels")]
		[SerializeField]
		private float m_panelResizableAreaLength = 12f;
		public float PanelResizableAreaLength { get { return m_panelResizableAreaLength; } }

		[Tooltip("size of the Dynamic Panels Canvas' drop zones. When a tab is dragged and dropped onto that area, it will be docked to that edge of the Dynamic Panels Canvas")]
		[SerializeField]
		private float m_canvasAnchorZoneLength = 20f;
		public float CanvasAnchorZoneLength { get { return m_canvasAnchorZoneLength; } }

		[Tooltip("size of the panels' drop zones. When a tab is dragged and dropped onto that area, it will be docked to that panel. This area is enabled only for docked panels (you can't dock panels to free panels)")]
		[SerializeField]
